...
export class AnswerSheetUploader {
    ...
    Promise.allSettled(Array.from(files).map(async (file: File) => {
      return new Promise<IAnswerSheet>(async (resolve, reject) => {
        ...
        this.managementService.addAnswerSheet(this.examId, matches[0], [file]).subscribe({
          ...
          error: error => {
            this.showWarning(error.error.error ? `student ${matches[0]} <${file.name}> ${error.error.error}` : undefined);
            reject(error)
          }
        });
      });
  ...
  async readZipArchive(file: File): Promise<void> {
    ...
    await Promise.allSettled(submitterDirectories
      .map(async (directory: ZipDirectoryEntry) => {
        ...
        const files: File[] = await Promise.all(directory.children
          .filter((child: ZipEntry) => !(child as ZipDirectoryEntry).directory && child.name.endsWith('.pdf'))
          .map(async (entry: ZipEntry) => new File([await (entry as ZipFileEntry<Blob, Blob>).getBlob()],entry.getFullname())));
        ...
        return new Promise<IAnswerSheet>((resolve, reject) => {
          this.managementService.addAnswerSheet(this.examId, studentIdMatches[0], files).subscribe({
            next: response => resolve(response),
            error: error => {
              this.showWarning(error.error.error ? `student ${studentIdMatches[0]} <${files.map(file => file.name).join('>, <')}> ${error.error.error}` : undefined);
              reject(error)
            }
          });
        });
