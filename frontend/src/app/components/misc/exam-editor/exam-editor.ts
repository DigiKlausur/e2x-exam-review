...
export class ExamEditor implements OnChanges {
  ...
  getFormValues(): IExam {
    ...
    if(formValues.reviewParameters.startDate === null) formValues.reviewParameters.startDate = this.exam?.reviewParameters.startDate ?? null;
    if(formValues.reviewParameters.endDate === null) formValues.reviewParameters.endDate = this.exam?.reviewParameters.endDate ?? null;
    if(formValues.reviewParameters.showDownloadButton === null) formValues.reviewParameters.showDownloadButton = this.exam?.reviewParameters.showDownloadButton ?? false;
    if(formValues.reviewParameters.showTextLayer === null) formValues.reviewParameters.showTextLayer = this.exam?.reviewParameters.showTextLayer ?? false;
    return formValues;
  }

  saveExam(): void {
    if(this.isNewExam) {
      this.managementService.createExam(this.getFormValues()).subscribe(response => {
        this.router.navigate(['/', 'manage', 'exam', response._id]);
      });
    }else{
      this.managementService.updateExam(this.getFormValues()).subscribe(response => {
