...
| `ARCHIVE_STORAGE_LOCATION`        | `./archive`                                  | location, where exam-data shall be exported to, after the automatic archiving period                                                      |
| `AUTOMATIC_ARCHIVING_PERIOD_DAYS` | `182`                                        | period in days since the exam date, after which exams shall be archived (setting this to `0` or `false` will disable automatic archiving) |
| `AUTOMATIC_ARCHIVING_TIME`        | `03:00`                                      | the time, at which automatic archiving shall be executed each day                                                                         |
